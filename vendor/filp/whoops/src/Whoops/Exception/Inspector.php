...
class Inspector
{
    private ?\Whoops\Exception\FrameCollection $frames = null;

    private ?\Whoops\Exception\Inspector $previousExceptionInspector = null;
    ...
    {
        return array_map(fn($prev) => /** @var \Throwable $prev */
$this->extractDocrefUrl($prev->getMessage())['message'], $this->getPreviousExceptions());
    }
    ...
    {
        return array_map(fn($prev) => /** @var \Throwable $prev */
$prev->getCode(), $this->getPreviousExceptions());
    }
    ...
    protected function isValidNextFrame(array $frame)
    {
        ...
        if (empty($frame['function']) || !stristr((string) $frame['function'], 'call_user_func')) {
            return false;
        }
